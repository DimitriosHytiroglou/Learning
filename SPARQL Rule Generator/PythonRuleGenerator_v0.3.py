...
with open(input("Give name or path of the CSV file: ")) as CSVTest:
	...
	for row in readCSV:
		print("Line: "+str(j)+", Rule: "+str(i))
		
		#MINIMUM CARDINALITY
		if row[2] in (None, ""):
			print ("ERROR")
			errors.append("Value missing in line "+str(j))
		elif row[2] == "1":
		...
		j+=1
	ClosingBrackets()
	PythTest.seek(tell,0)
	PythTest.write("      ")

	print("\n====================================================================")
	print("\nNumber of rules generated: "+str(i)+"\n")
	if errors != "":
...
#
# Modify the error detection feature to be a separate function that stores different kinds of values accordingly
